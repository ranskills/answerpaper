...
- `dialog.js` — `showConfirm()`/`showAlert()`, a Promise-based replacement for `window.confirm()`/`alert()` built on a single lazily-created native `<dialog>` element (see "Design conventions to preserve" below). Loaded after `i18n.js` (it calls `t()` for default button labels) and before `render.js`.
