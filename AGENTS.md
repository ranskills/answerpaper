...
- `logic.js` — pure functions only, no DOM access: grading (`gradeResponse`), regrading (`setCorrectAnswer`), trend computation (`computeQuestionTrend`, `computeChapterTrend`, `weakestQuestions`), and the Home dashboard aggregates (`computeOverallStats`, `computeStudyStreak`, `computeContinueChapter`, `computeNeedsAttention`). Classic script.
