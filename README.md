...
No backend, no login, no build step — it's a handful of static files you can open directly in a browser or host on GitHub Pages. The one library it uses, Preact + htm, is vendored directly into the repo (`assets/js/vendor/`, see that folder's README for provenance), so there's nothing to install or fetch from npm/a CDN — "no dependencies" here means nothing you need to set up, not that the app has zero library code.
...
- **Resume where you left off.** Closing the tab or reloading mid-attempt doesn't lose your progress — a "Resume attempt" button on Home and the chapter detail page picks the wizard back up right where you were.
